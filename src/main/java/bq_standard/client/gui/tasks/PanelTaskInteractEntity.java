...
public class PanelTaskInteractEntity extends CanvasEmpty {
  ...
  public void initPanel() {
    ...
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 32, -40, 16, 16, 0), PresetIcon.ICON_RIGHT.getTexture()));

    UUID playerID = QuestingAPI.getQuestingUUID(Minecraft.getMinecraft().player);
    int prog = task.getUsersProgress(playerID);
    this.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.MID_LEFT, 0, -14, 32, 14, 0),
                                   prog + "/" + task.required).setAlignment(1)
                                                              .setColor(PresetColor.TEXT_MAIN.getColor()));

    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 0, 0, 24, 24, 0), BQSTextures.HAND_LEFT.getTexture()));
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 24, 0, 24, 24, 0), BQSTextures.HAND_RIGHT.getTexture()));
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 0, 24, 24, 24, 0), BQSTextures.ATK_SYMB.getTexture()));
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 24, 24, 24, 24, 0), BQSTextures.USE_SYMB.getTexture()));
    ...
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 16, 16, 8, 8, 0), task.useOffHand ? txTick : txCross));
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 40, 16, 8, 8, 0), task.useMainHand ? txTick : txCross));
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 16, 40, 8, 8, 0), task.onHit ? txTick : txCross));
    this.addPanel(
        new PanelGeneric(new GuiTransform(GuiAlign.MID_LEFT, 40, 40, 8, 8, 0), task.onInteract ? txTick : txCross));
    ...
    if (target != null) {
      this.addPanel(
          new PanelEntityPreview(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(48, 0, 0, 0), 0),
