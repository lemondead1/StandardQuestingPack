...
public class PanelTaskLocation extends CanvasEmpty {
  ...
  public void initPanel() {
    ...
    if (!task.hideInfo) {
      ...
      if (task.range >= 0) {
        ...
        desc += "\n" + QuestTranslation.translate("bq_standard.gui.distance",
                                                  (int) Minecraft.getMinecraft().player.getDistance(task.x, task.y,
                                                                                                    task.z) + "m");
      }
    ...
    this.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 0, 0, 0), 0), desc).setColor(
    ...
    PanelGeneric panelCompass =
        new PanelGeneric(new GuiTransform(GuiAlign.BOTTOM_RIGHT, -innerSize, -innerSize, innerSize, innerSize, -1),
