...
public class PanelRewardRecipe extends CanvasEmpty {
  ...
  public void initPanel() {
    ...
    for (String s : reward.recipeNames.split("\n")) {
      ...
      PanelItemSlot is =
          new PanelItemSlot(new GuiRectangle(x * iconSize, y * iconSize, iconSize, iconSize, 0), -1, stack, true);
      cvList.addPanel(is);
    }

    if (dynamic > 0) {
      cvList.addPanel(
          new PanelTextBox(new GuiRectangle(0, (entry / rowSize) * iconSize + 4, listWidth, 14, 0),
                           "+" + dynamic + " more...").setColor(PresetColor.TEXT_MAIN.getColor()));
    }
