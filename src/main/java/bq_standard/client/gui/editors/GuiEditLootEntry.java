...
public class GuiEditLootEntry extends GuiScreenCanvas {
  ...
  public void initPanel() {
    ...
    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 16, 16, -32), 0),
                                           QuestTranslation.translate(
                                               "bq_standard.title.edit_loot_groups")).setAlignment(1).setColor(
    ...
    fieldName = new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 16, 0, -32), 0),
                                 selEntry != null ? "#" + selectedID : "#--").setColor(
        PresetColor.TEXT_MAIN.getColor());
    ...
    cvRight.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 36, 0, -48), 0),
                                      QuestTranslation.translate("bq_standard.gui.weight")).setColor(
        PresetColor.TEXT_MAIN.getColor()));

    fieldWeight =
        new PanelTextField<>(new GuiTransform(new Vector4f(0F, 0F, 0.5F, 0F), new GuiPadding(0, 48, 0, -64), 0),
                             "" + (selEntry != null ? selEntry.weight : 1), FieldFilterNumber.INT);
    ...
    textWeight = new PanelTextBox(new GuiTransform(new Vector4f(0.5F, 0F, 1F, 0F), new GuiPadding(4, 52, 0, -64), 0),
                                  "/1 (100%)").setColor(PresetColor.TEXT_MAIN.getColor());
      ...
      public void onButtonClick() {
        if (selEntry != null) {
          ...
          mc.displayGuiScreen(QuestingAPI.getAPI(ApiReference.THEME_REG).getGui(PresetGUIs.EDIT_NBT,
                                                                                new GArgsNBT<>(screenRef,
                                                                                               eTag.getTagList("items",
                                                                                                               10),
                                                                                               value -> {
                                                                                                 LootGroup lg =
                                                                                                     LootRegistry.INSTANCE.getValue(
                                                                                                         groupID);
                                                                                                 LootEntry le =
                                                                                                     lg == null ? null
                                                                                                                : lg.getValue(
  ...
  private void refreshEntries() {
    ...
    for (int i = 0; i < lgAry.size(); i++) {
      lootList.addPanel(
          new PanelButtonStorage<>(new GuiRectangle(0, i * 16, 16, 16, 0), -1, "", lgAry.get(i)).setCallback(value ->
                                                                                                             {
                                                                                                               lootGroup.removeID(
                                                                                                                   value.getID());
                                                                                                               refreshEntries();
      ...
      lootList.addPanel(
          new PanelButtonStorage<>(new GuiRectangle(16, i * 16, lWidth - 16, 16, 0), -1, "#" + lgAry.get(i).getID(),
                                   lgAry.get(i)).setCallback(value ->
                                                             {
                                                               ...
                                                               int totalWeight = lootGroup.getTotalWeight();
                                                               float chance =
                                                                   selEntry.weight / (float) totalWeight * 100F;
                                                               textWeight.setText(
                                                                   "/" + totalWeight + " (" + numFormat.format(chance) +
                                                                   "%)");
                                                             }));
    }
  }
