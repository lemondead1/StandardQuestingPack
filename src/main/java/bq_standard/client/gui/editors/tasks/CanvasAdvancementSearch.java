...
public abstract class CanvasAdvancementSearch extends CanvasSearch<Advancement, Advancement> {
  ...
  protected void queryMatches(Advancement value, String query, ArrayDeque<Advancement> results) {
    if (value.getId().toString().toLowerCase().contains(query.toLowerCase())) {
      results.add(value);
    } else if (value.getDisplay() != null &&
               value.getDisplay().getTitle().getFormattedText().toLowerCase().contains(query.toLowerCase())) {
      results.add(value);
  ...
  private static final Comparator<Advancement> advComparator = (o1, o2) -> {
    ...
    String s1 = o1.getDisplay() == null ? o1.getId().toString().toLowerCase()
                                        : o1.getDisplay().getTitle().getFormattedText().toLowerCase();
    String s2 = o2.getDisplay() == null ? o2.getId().toString().toLowerCase()
                                        : o2.getDisplay().getTitle().getFormattedText().toLowerCase();
