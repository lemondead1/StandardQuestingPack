...
public class GuiEditTaskTame extends GuiScreenCanvas implements IVolatileScreen {
  ...
  public void initPanel() {
    ...
    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 16, 16, -32), 0),
                                           QuestTranslation.translate("bq_standard.title.edit_tame")).setAlignment(1)
                                                                                                     .setColor(
    ...
    this.addPanel(new PanelEntityPreview(new GuiTransform(GuiAlign.HALF_TOP, new GuiPadding(16, 32, 16, 0), 0),
                                         target).setRotationDriven(new ValueFuncIO<>(() -> 15F), new ValueFuncIO<>(
        () -> (float) (Minecraft.getSystemTime() % 30000L / 30000D *
                       360D)))); // Preview works with null. It's fine (or should be)

    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.MID_CENTER, -100, 4, 96, 12, 0),
                                           QuestTranslation.translate("bq_standard.gui.amount")).setAlignment(2)
                                                                                                .setColor(
                                                                                                    PresetColor.TEXT_MAIN.getColor()));
    cvBackground.addPanel(
        new PanelTextField<>(new GuiTransform(GuiAlign.MID_CENTER, 0, 0, 100, 16, 0), "" + task.required,
    ...
    cvBackground.addPanel(new PanelButton(new GuiTransform(GuiAlign.MID_CENTER, -100, 32, 200, 16, 0), -1,
