  ...
  public void initPanel() {
    ...
    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 16, 16, -32), 0),
                                           QuestTranslation.translate(
                                               "bq_standard.title.edit_advancement")).setAlignment(1).setColor(
        PresetColor.TEXT_HEADER.getColor()));

    CanvasAdvancementSearch cvAdvList =
        new CanvasAdvancementSearch(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(16, 48, 24, 24), 0),
          ...
          protected boolean addResult(Advancement entry, int index, int cachedWidth) {
            ...
            this.addPanel(
                new PanelGeneric(new GuiRectangle(0, index * 24, 24, 24, 0), PresetTexture.ITEM_FRAME.getTexture()));
            if (disp != null) {
              this.addPanel(new PanelGeneric(new GuiRectangle(0, index * 24, 24, 24, -1),
                                             new ItemTexture(new BigItemStack(disp.getIcon()))));
            }

            PanelButtonStorage<Advancement> btnAdv =
                new PanelButtonStorage<>(new GuiRectangle(24, index * 24, cachedWidth - 24, 24, 0), -1,
            ...
            if (disp != null) {
              btnAdv.setTooltip(
                  RenderUtils.splitString(disp.getDescription().getFormattedText(), 128, mc.fontRenderer));
            }
    ...
    PanelVScrollBar scAdv =
        new PanelVScrollBar(new GuiTransform(GuiAlign.RIGHT_EDGE, new GuiPadding(-24, 48, 16, 24), 0));
    cvBackground.addPanel(scAdv);
    cvAdvList.setScrollDriverY(scAdv);

    PanelTextField<String> tfSearch =
        new PanelTextField<>(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 32, 16, -48), 0), "",
