...
public class GuiEditTaskScoreboard extends GuiScreenCanvas {
  ...
  public void initPanel() {
    ...
    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(16, 16, 16, -32), 0),
                                           QuestTranslation.translate(
                                               "bq_standard.title.edit_scoreboard")).setAlignment(1).setColor(
        PresetColor.TEXT_HEADER.getColor()));

    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.MID_CENTER, -100, -28, 50, 12, 0),
                                           QuestTranslation.translate("betterquesting.gui.name")).setColor(
        PresetColor.TEXT_MAIN.getColor()));
    cvBackground.addPanel(new PanelTextBox(new GuiTransform(GuiAlign.MID_CENTER, -100, -12, 50, 12, 0), "ID").setColor(
        PresetColor.TEXT_MAIN.getColor())); // TODO: Localise this?

    cvBackground.addPanel(
        new PanelTextField<>(new GuiTransform(GuiAlign.MID_CENTER, -50, -32, 150, 16, 0), task.scoreDisp,
                             FieldFilterString.INSTANCE).setCallback(value -> task.scoreDisp = value));
    cvBackground.addPanel(
        new PanelTextField<>(new GuiTransform(GuiAlign.MID_CENTER, -50, -16, 150, 16, 0), task.scoreName,
                             FieldFilterString.INSTANCE).setCallback(value -> task.scoreName = value));

    cvBackground.addPanel(
        new PanelButtonStorage<ScoreOperation>(new GuiTransform(GuiAlign.MID_CENTER, -100, 0, 50, 16, 0), -1,
                                               task.operation.GetText(), task.operation) {
          @Override
    ...
    cvBackground.addPanel(
        new PanelTextField<>(new GuiTransform(GuiAlign.MID_CENTER, -50, 0, 150, 16, 0), "" + task.target,
                             FieldFilterNumber.INT).setCallback(value -> task.target = value));

    final GuiScreen screenRef = this;
    cvBackground.addPanel(new PanelButton(new GuiTransform(GuiAlign.MID_CENTER, -100, 16, 200, 16, 0), -1,
                                          QuestTranslation.translate("betterquesting.btn.advanced")) {
      @Override
      public void onButtonClick() {
        mc.displayGuiScreen(QuestingAPI.getAPI(ApiReference.THEME_REG).getGui(PresetGUIs.EDIT_NBT,
                                                                              new GArgsNBT<>(screenRef, task.writeToNBT(
                                                                                  new NBTTagCompound()),
                                                                                             task::readFromNBT, null)));
      }
    });
  ...
  // TODO: Really need to make the native packet types accessible in the API
