...
public class TaskFluid implements ITaskInventory, IFluidTask, IItemTask {
  private final Set<UUID> completeUsers = new HashSet<>();
  ...
  public void detect(ParticipantInfo pInfo, DBEntry<IQuest> quest) {
    ...
    final List<Tuple<UUID, int[]>> progress =
      ...
      for (int i = 0; i < invo.getSizeInventory(); i++) {
        ...
        for (int j = 0; j < requiredFluids.size(); j++) {
          ...
          for (Tuple<UUID, int[]> value : progress) {
            ...
            FluidStack fluid =
                handler.drain(drain, consume); // TODO: Look into reducing this to a single call if possible
  ...
  public void readProgressFromNBT(NBTTagCompound nbt, boolean merge) {
    ...
    for (int n = 0; n < pList.tagCount(); n++) {
      try {
        ...
        for (int i = 0; i < data.length && i < dNbt.tagCount();
             i++) // TODO: Change this to an int array. This is dumb...
        {
  ...
  @Override
  public FluidStack submitFluid(UUID owner, DBEntry<IQuest> quest, FluidStack fluid, boolean doSubmit) {
    return submitFluidInternal(owner, quest, fluid, doSubmit);
  }
  ...
  @Override
  public ItemStack submitItem(UUID owner, DBEntry<IQuest> quest, ItemStack input, boolean doSubmit) {
    ...
    IFluidHandlerItem handler = Objects.requireNonNull(FluidUtil.getFluidHandler(item));

    for (FluidStack fluid : requiredFluids) {
      FluidStack tryDrain = handler.drain(fluid, false);
      if (tryDrain == null) {
        continue;
      }
      FluidStack remainder = submitFluidInternal(owner, quest, tryDrain, false);
      tryDrain.amount -= remainder == null ? 0 : remainder.amount;
      if (tryDrain.amount == 0) {
        continue;
      }
      FluidStack drained = handler.drain(tryDrain, true);
      if (drained == null || drained.amount == 0) {
        continue;
      }
      if (doSubmit) {
        submitFluidInternal(owner, quest, drained, true);
      }
    }
