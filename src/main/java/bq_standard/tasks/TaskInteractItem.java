...
public class TaskInteractItem implements ITask {
  ...
  private final Map<UUID, Integer> userProgress = new ConcurrentHashMap<>();
  ...
  public void onInteract(ParticipantInfo pInfo, DBEntry<IQuest> quest, EnumHand hand, ItemStack item, IBlockState state,
                         BlockPos pos, boolean isHit) {
    ...
    if (targetBlock.b != Blocks.AIR) {
      ...
      boolean oreMatch = targetBlock.oreDict.length() > 0 && OreDictionary.getOres(targetBlock.oreDict).contains(
          new ItemStack(state.getBlock(), 1, tmpMeta));
    ...
    if (targetItem.getBaseStack().getItem() != Items.AIR) {
      if (targetItem.hasOreDict() &&
          !ItemComparison.OreDictionaryMatch(targetItem.getOreIngredient(), targetItem.GetTagCompound(), item,
