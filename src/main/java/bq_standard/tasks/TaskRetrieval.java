...
public class TaskRetrieval implements ITaskInventory, IItemTask {
  ...
  private final Map<UUID, int[]> userProgress = new ConcurrentHashMap<>();
      ...
      for (int i = 0; i < invo.getSizeInventory(); i++) {
        ...
        for (int j = 0; j < requiredItems.size(); j++) {
          ...
          if (!ItemComparison.StackMatch(rStack.getBaseStack(), stack, !ignoreNBT, partialMatch) &&
              !ItemComparison.OreDictionaryMatch(rStack.getOreIngredient(), rStack.GetTagCompound(), stack, !ignoreNBT,
                                                 partialMatch)) {
            continue;
          }
  ...
  public ItemStack submitItem(UUID owner, DBEntry<IQuest> quest, ItemStack input, boolean doSubmit) {
    ...
    if (doSubmit && updated) {
      ...
      if (player != null) {
        ...
        boolean hasAll = true;
        for (int j = 0; j < requiredItems.size(); j++) {
          if (progress[j] >= requiredItems.get(j).stackSize) {
            continue;
          }
