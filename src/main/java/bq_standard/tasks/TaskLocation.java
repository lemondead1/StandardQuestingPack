...
public class TaskLocation implements ITaskTickable {
  ...
  private void internalDetect(@Nonnull ParticipantInfo pInfo, DBEntry<IQuest> quest) {
    ...
    if (playerMP.dimension == dim && (range <= 0 || getDistance(playerMP) <= range)) {
      if (!StringUtils.isNullOrEmpty(biome) && !new ResourceLocation(biome).equals(
          playerMP.getServerWorld().getBiome(playerMP.getPosition()).getRegistryName())) {
        if (!invert) { return; }
      } else if (!StringUtils.isNullOrEmpty(structure) && !playerMP.getServerWorld().getChunkProvider()
                                                                   .isInsideStructure(playerMP.world, structure,
                                                                                      playerMP.getPosition())) {
        if (!invert) { return; }
