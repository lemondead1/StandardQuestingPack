...
public class AdvImporter implements IImporter {
        ...
        try {
          ...
          while (iterator.hasNext()) {
            ...
            ResourceLocation advID =
                new ResourceLocation(entry.getKey(), FilenameUtils.removeExtension(relPath).replaceAll("\\\\", "/"));
  ...
  private ResourceLocation guessID(File file) {
    ...
    while (parent != null) {
      if (parent.getName().equalsIgnoreCase("advancements") ||
          (parent.getName().equalsIgnoreCase("json") && parent.getParentFile() != null &&
           parent.getParentFile().getName().equalsIgnoreCase("triumph"))) {
  ...
  private final TreeMap<ResourceLocation, DBEntry<IQuest>> ID_MAP =
      new TreeMap<>((o1, o2) -> o2.toString().compareToIgnoreCase(o1.toString()));
  // Reverse sort... because Minecraft does (I think?).
    ...
    if (json.has("rewards")) {
      ...
      if (rewObj.has("loot")) {
        ...
        for (int i = 0; i < ary.size(); i++) {
          BigItemStack item = new BigItemStack(BQ_Standard.lootChest, 1, 103);
      ...
      for (AdvTreeNode node : NODES_BY_DEPTH.get(d)) {
        ...
        while (childStack.size() > 0) {
          ...
          if (changed && child.getChildren().size() > 0) {
            for (int i = child.getChildren().size() - 1; i >= 0; i--) { childStack.push(child.getChildren().get(i)); }
          }
        }
      }
    }
