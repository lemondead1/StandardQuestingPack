  ...
  */
  public ITask[] convertTask(NBTTagCompound nbt) {
    ...
    task.consume = nbt.getBoolean(
        "consume_items"); // If the default were changed to true and this was redacted then too bad. I'm not going looking for the root file just for this task
    long count = !nbt.hasKey("count") ? 1 : nbt.getLong(
        "count"); // Why this isn't per item I have no idea. Ask the FTBQ dev. Also not a fan of supporting stack counts in excess of 2 BILLION items.
