  ...
  public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, @Nonnull EnumHand hand) {
    ...
    if (stack.getItemDamage() == 104) {
      ...
      LootContext lootcontext =
          (new LootContext.Builder(((EntityPlayerMP) player).getServerWorld())).withLootedEntity(player)
                                                                               .withPlayer(player)
                                                                               .withLuck(player.getLuck()).build();
      String loottable =
          (stack.getTagCompound() != null && stack.getTagCompound().hasKey("loottable", 8)) ? stack.getTagCompound()
                                                                                                   .getString(
                                                                                                       "loottable")
      ...
      float rarity =
          stack.getItemDamage() == 101 ? itemRand.nextFloat() : MathHelper.clamp(stack.getItemDamage(), 0, 100) / 100F;
