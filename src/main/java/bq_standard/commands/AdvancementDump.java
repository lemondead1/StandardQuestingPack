...
public class AdvancementDump {
  ...
  private final HashMap<Advancement, DBEntry<IQuest>> idMap = new HashMap<>();
  ...
  public void dumpAdvancements(MinecraftServer server) {
    ...
    for (Advancement adv : server.getAdvancementManager().getAdvancements()) {
      ...
      quest.setProperty(NativeProps.SILENT,
                        true); // There's already toast notifications. Double stacking would just be annoying
      quest.setProperty(NativeProps.VISIBILITY,
                        (disp.isHidden() || adv.getParent() == null) ? EnumQuestVisibility.COMPLETED
                                                                     : EnumQuestVisibility.UNLOCKED);
